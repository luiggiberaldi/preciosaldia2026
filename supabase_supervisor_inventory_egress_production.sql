-- MIGRACIÓN DE PRODUCCIÓN — EGRESO DE STOCK DEL SUPERVISOR
--
-- Objetivo: ampliar el comando de ajuste por lote para permitir ingreso y egreso.
-- Esta migración no aplica ningún movimiento; solo abre la autorización backend.
-- Activar el frontend únicamente después de verificar la migración y el rollback.
--
-- ROLLBACK:
-- 1) configurar VITE_SUPERVISOR_REMOTE_EGRESS_ENABLED=false en Vercel;
-- 2) restaurar supabase_supervisor_inventory_income_production.sql;
-- 3) verificar 0 comandos pending;
-- 4) revisar cualquier AJUSTE_SALIDA aplicado mediante auditoría.
...
BEGIN
    ...
    -- Validar la duración contra issued_at para evitar rechazos por latencia.
