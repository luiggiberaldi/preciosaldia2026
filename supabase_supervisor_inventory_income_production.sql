-- MIGRACIÓN DE PRODUCCIÓN — CANARY DE INGRESO DE STOCK DEL SUPERVISOR
--
-- Objetivo: permitir únicamente supervisor.inventory.batch.adjust con direction=ingreso.
-- No habilita el flag frontend ni permite egresos.
-- Ejecutar solo después de aprobar M5 y conservar el rollback.
--
-- ROLLBACK:
-- 1) volver SUPERVISOR_REMOTE_MUTATIONS_ENABLED a false;
-- 2) retirar supervisor.inventory.batch.adjust del CHECK;
-- 3) restaurar la RPC anterior de create_supervisor_command;
-- 4) verificar 0 comandos pending.
